...
module.exports = async function (client, message, args, prefix) {
  ...
  **34: ** [\`${Radiostations[34-1].split(" ")[0]}\`](${Radiostations[34-1].split(" ")[1]})`, inline: true },
  ...
  const song = {
    title: args2[0],
    url: args2[1],
    thumbnail: botsettings.gif2,
    duration: 10000,
  };

  
  let connection = await channel.join().then(message.channel.send(`👍 **joined** \`${message.member.voice.channel.name}\``));
  ...
  dispatcher.on("end", end => {radioexecuteadmin()});
    ...
    console.log(error);
    await channel.leave();
    return message.channel.send(`Could not join the channel: ${error}`).catch(e => console.log(e));
  }
  }catch(e){
    console.log(e)
  }
